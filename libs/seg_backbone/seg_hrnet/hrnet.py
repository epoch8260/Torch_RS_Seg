...
def _hrnet(arch, pretrained, **kwargs):
    try:
        from ..config.hrnet_config import MODEL_CONFIGS
    except ImportError:
        from libs.nn.seg_backbone.seg_hrnet.hrnet_config import MODEL_CONFIGS
    model = HighResolutionNet(MODEL_CONFIGS[arch], **kwargs)
    if pretrained:
        ckpt_folder = os.path.split(os.path.realpath(__file__))[0]
        model_urls = {'hrnet18': os.path.join(ckpt_folder, 'hrnetv2_w18_imagenet_pretrained.pth'),
                      'hrnet32': os.path.join(ckpt_folder, 'hrnetv2_w32_imagenet_pretrained.pth'),
                      'hrnet40': os.path.join(ckpt_folder, 'hrnetv2_w40_imagenet_pretrained.pth'),
                      'hrnet48': os.path.join(ckpt_folder, 'hrnetv2_w48_imagenet_pretrained.pth')
                      }
...
def hrnet18(pretrained=True, **kwargs):
    r"""HRNet-18 model
    """
    return _hrnet('hrnet18', pretrained, **kwargs)


def hrnet32(pretrained=True, **kwargs):
    r"""HRNet-32 model
    """
    return _hrnet('hrnet32', pretrained, **kwargs)


def hrnet40(pretrained=True, **kwargs):
    r"""HRNet-40 model
    """
    return _hrnet('hrnet40', pretrained, **kwargs)


def hrnet48(pretrained=True, **kwargs):
    r"""HRNet-48 model
    """
    return _hrnet('hrnet48', pretrained, **kwargs)


if __name__ == '__main__':
    model = hrnet40(pretrained=True)
