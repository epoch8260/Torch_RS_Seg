...
class RepVGGBlock(nn.Module):
    ...
    def get_custom_L2(self):
        ...
        l2_loss_circle = (K3 ** 2).sum() - (
                K3[:, :, 1:2, 1:2] ** 2).sum()  # The L2 loss of the "circle" of weights in 3x3 kernel. Use regular L2 on them.
...
def create_RepVGG_A0(deploy=False, pretrained=False):
    train_model = RepVGG(num_blocks=[6, 4, 6, 1], num_classes=1000,
                         width_multiplier=[0.75, 0.75, 0.75, 2.5], override_groups_map=False, deploy=deploy)
    if pretrained:
        train_model.load_state_dict(torch.load('RepVGG-A0-train.pth'))  # or train from
    return train_model


func_dict = {
    'RepVGG-A0': create_RepVGG_A0  # Updated at April 25, 2021. This is not reported in the CVPR paper.
}
